-- Extend real segment reconciliation with optimistic concurrency and sync metadata.
...
create or replace function reconcile_real_execution_segments(
  ...
  p_sync_mutation_id text default null
)
returns jsonb
language plpgsql
...
declare
  ...
  v_existing_processed sync_processed_mutations%rowtype;
  ...
  v_deleted_segment jsonb;
  v_field_id bigint;
  v_option_id bigint;
  v_value_text text;
begin
  if p_sync_mutation_id is not null and btrim(p_sync_mutation_id) <> '' then
    select *
      into v_existing_processed
      from sync_processed_mutations
     where mutation_id = p_sync_mutation_id
       and domain = 'planning'
       and scope_user_id is null
     limit 1;

    if found then
      return v_existing_processed.response;
    end if;
  end if;
  ...
  for v_deleted_segment in
    select to_jsonb(segment_row)
    from activity_execution_segments segment_row
    where segment_row.id = any(v_extra_ids)
  loop
    ...
    values (
      ...
      v_deleted_segment ->> 'id',
      'delete',
      v_deleted_segment ->> 'updated_at',
      jsonb_build_object(
        'id', (v_deleted_segment ->> 'id')::bigint,
        'tracking_type', 'real',
        'item_date', v_deleted_segment ->> 'item_date'
      ),
      nullif(p_sync_mutation_id, ''),
      p_actor_user_id
    );
  ...
  if p_sync_mutation_id is not null and btrim(p_sync_mutation_id) <> '' then
    ...
    values (
      ...
      'activity_execution_segment',
      coalesce((v_result_ids[1])::text, p_segment_id::text),
      (select max(updated_at)::text from activity_execution_segments where id = any(v_result_ids)),
      v_after_data,
      p_actor_user_id
    );
  end if;
...
revoke all on function reconcile_real_execution_segments(bigint, bigint, text, jsonb, jsonb, uuid, text, uuid, timestamptz, text) from public;
revoke all on function reconcile_real_execution_segments(bigint, bigint, text, jsonb, jsonb, uuid, text, uuid, timestamptz, text) from authenticated;
grant execute on function reconcile_real_execution_segments(bigint, bigint, text, jsonb, jsonb, uuid, text, uuid, timestamptz, text) to service_role;
